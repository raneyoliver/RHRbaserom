...
!CloneHighSpin =            $FC
!CloneLowSpin =             $FE
!CloneJumpHeld =        $7FA018 ;jumpHeld in npc2.asm TODO: change to sprite table
!CloneSpinning =        $7FA00C ;free ram for !Spinning in npc2.asm
...
+   jsr HandleStunned       ;> Handle stun timer stuff.
    ...
    LDA !14C8,x
    CMP #$0B
    BEQ .skipSpriteInteraction
    jsl $018032|!bank       ;> Interact with sprites.

.skipSpriteInteraction
    lda $1419|!addr         ;\
    bne +                   ;|
...
;.left
;    CMP #$E7
;    BCC ..playerLeft  ;fast? drop in player moving direction
;
;    JMP GetThrowDirection   ;slow? drop l/r if holding l/r
;
;..playerLeft
;    LDY #$00
;    RTS
;.right
;    CMP #$18          ;fast? drop in player moving direction
;    BCS ..playerRight
;
;    JMP GetThrowDirection   ;slow? drop l/r if holding l/r
;
...
.left
    LDA $15
    AND #$02
    BEQ .throwBasedOnMarioDirection ;not holding left

    LDY #$00    ;kick left
    BRA .re

.throwBasedOnMarioDirection
    LDY $76     ;mario direction
.re
    RTS
